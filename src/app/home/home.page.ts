...
export class HomePage implements OnInit {
  ...
  findAcceptedOrdersAndAccepted(id: number) {
    const index = this.items.findIndex(e => e.id === id)
    if (index >= 0) {
      const indexuser = this.items[index].orders_accepted.findIndex((user: {
        status_order: Boolean | undefined;
        id: number | undefined;
      }) => user.id === this.authService.currentUser?.id && user.status_order)
      return indexuser < 0;
      ...
      buttons: [
        {
          ...
          handler: async () => {
            ...
            if (res.status) {
              // this.items = await this.authService.getMyOrders().toPromise();
              this.getOrders()
            }
          }
        }
  ...
  filterOrders() {
    this.getOrders()
    this.items = this.items.filter((item) => {
      return item.route.from_city.toLowerCase().includes(this.filteredCityOut.toLowerCase());
    });
    this.filter = false;
  }
  filterClose() {
